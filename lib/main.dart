    ...
    return Scaffold(
      ...
      body: _currentIndex == 0
          ? AnalysePage(
              ...
              objectifs: objectifs,
              onObjectifChanged: (val) => setState(() => selectedObjectif = val),
            )
...
class AnalysePage extends StatefulWidget {
  ...
  const AnalysePage({
    super.key,
    required this.selectedObjectif,
  ...
  Widget _buildResults() {
    ...
    if (analysisData!.containsKey('error')) {
      return Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFF2E1A1A), borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.red.withOpacity(0.4))),
        child: Text(analysisData!['error'], style: const TextStyle(color: Colors.white)),
      );
    }
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _statCard("GAUCHE MIN", "${d['knee_min_left']}°", Icons.rotate_left, const Color(0xFF4CAF50))),
          const SizedBox(width: 10),
          Expanded(child: _statCard("DROIT MIN", "${d['knee_min_right']}°", Icons.rotate_right, const Color(0xFF4CAF50))),
        ]),
        const SizedBox(height: 12),
        const Text("ANGLES HANCHE", style: TextStyle(fontSize: 11, color: Colors.white38, fontWeight: FontWeight.w600, letterSpacing: 1)),
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _statCard("GAUCHE MOY", "${d['hip_avg_left']}°", Icons.rotate_left, const Color(0xFFFF9800))),
          const SizedBox(width: 10),
          Expanded(child: _statCard("DROIT MOY", "${d['hip_avg_right']}°", Icons.rotate_right, const Color(0xFFFF9800))),
        ]),
        const SizedBox(height: 12),
        const Text("POSTURE", style: TextStyle(fontSize: 11, color: Colors.white38, fontWeight: FontWeight.w600, letterSpacing: 1)),
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _statCard("ANGLE DOS", "${d['back_avg']}°", Icons.accessibility_new, const Color(0xFF2196F3))),
          const SizedBox(width: 10),
          Expanded(child: _statCard("SYMÉTRIE", "${d['symmetry']}°", Icons.compare_arrows, const Color(0xFF2196F3))),
        ]),
        const SizedBox(height: 12),
        const Text("STABILITÉ GENOUX", style: TextStyle(fontSize: 11, color: Colors.white38, fontWeight: FontWeight.w600, letterSpacing: 1)),
        const SizedBox(height: 8),
        Row(children: [
          Expanded(child: _statCard("GAUCHE", "${d['knee_stability_left']}", Icons.show_chart, const Color(0xFFE91E63))),
          const SizedBox(width: 10),
          Expanded(child: _statCard("DROIT", "${d['knee_stability_right']}", Icons.show_chart, const Color(0xFFE91E63))),
        ]),
        ...
        if (d['coaching'] != null)
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 16),
              Text(d['coaching'], style: TextStyle(fontSize: 14, color: Colors.white.withOpacity(0.85), height: 1.6)),
            ]),
          ),
      ],
    );
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.fitness_center, size: 48, color: Color(0xFF6C63FF)),
            ),
            const SizedBox(height: 16),
            ...
            Row(
              children: widget.objectifs.map((o) {
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: isSelected ? const Color(0xFF6C63FF) : Colors.white.withOpacity(0.1)),
                      ),
            ...
            GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: videoBytes != null ? const Color(0xFF6C63FF).withOpacity(0.15) : const Color(0xFF1A1A2E),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: videoBytes != null ? const Color(0xFF6C63FF) : Colors.white.withOpacity(0.1), width: 1.5),
                ),
                child: Column(children: [
                  ...
                  if (videoBytes != null) ...[
                    ...
                    Text("Appuie pour changer", style: TextStyle(fontSize: 12, color: Colors.white.withOpacity(0.3))),
                  ],
                  const SizedBox(height: 8),
              ...
              child: ElevatedButton(
                ...
                child: isLoading
                    ? const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        ...
                        SizedBox(width: 12),
                        Text("Analyse en cours...", style: TextStyle(color: Colors.white, fontSize: 16)),
                      ])
...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> sendMessage() async {
    ...
    setState(() {
      messages.add({
        "role": "user",
        "text": text.isNotEmpty ? text : "Analyse cette vidéo",
        "video": pendingVideoName,
      });
      isLoading = true;
    });
    _controller.clear();
    ...
    try {
      ...
      setState(() {
        messages.add({"role": "assistant", "text": json['response']});
        isLoading = false;
      });
      _scrollToBottom();
    } catch (e) {
      setState(() {
        messages.add({"role": "assistant", "text": "Erreur de connexion au serveur."});
        isLoading = false;
      });
    }
  }
                          ...
                          children: [
                            if (!isUser) ...[
                              Container(
                                ...
                                child: const Icon(Icons.psychology, size: 16, color: Color(0xFF6C63FF)),
                              ),
                            ...
                            Flexible(
                              child: Container(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  ...
                                  Text(msg['text'], style: TextStyle(fontSize: 14, color: Colors.white.withOpacity(isUser ? 1 : 0.85), height: 1.5)),
                                ]),
                              ),
                            ),
                          ],
                  ...
                  children: [
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: isLoading ? const Color(0xFF6C63FF).withOpacity(0.3) : const Color(0xFF6C63FF),
